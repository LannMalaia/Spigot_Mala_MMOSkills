...
public class Haste_Wave extends RegisteredSkill
{
	public Haste_Wave()
	{	
		super(new Haste_Wave_Handler(), MalaMMO_Skill.plugin.getConfig());
...
class Haste_Wave_Handler extends MalaSkill implements Listener
{
	public Haste_Wave_Handler()
	{
		super(	"HASTE_WAVE",
				"에리어 헤이스트",
				Material.MUSIC_DISC_13,
				"&7자신을 포함한 주변 &8{distance}&7 거리에 헤이스트를 부여합니다.",
				"&7이 때 효과는 기존 헤이스트를 따릅니다.",
				"&8헤이스트를 배우지 않았다면 사용할 수 없습니다.",
				"",
	...
	public SimpleSkillResult getResult(SkillMetadata cast)
	{
		...
		if(!Skill_Util.Has_Skill(data, "HASTE", 1))
		{
	...
	public void whenCast(SimpleSkillResult _data, SkillMetadata cast)
	{
		...
		RegisteredSkill skill = MMOCore.plugin.skillManager.getSkill("HASTE");
		...
		Bukkit.getScheduler().runTask(MalaMMO_Skill.plugin, new HasteWaveEffect(data.getPlayer(), distance));

		// 자신에게도 적용
		Haste_Handler.Haste_Target(data.getPlayer(), amp, tick);
		
		for(Entity e : data.getPlayer().getNearbyEntities(distance, distance, distance))
		{
			if(e instanceof Player)
			{
				e.sendMessage("§b" + data.getPlayer().getDisplayName() + "§b님으로부터 가속을 부여받았습니다.");
				Haste_Handler.Haste_Target((Player)e, amp, tick);
			}
		}
...
class HasteWaveEffect implements Runnable
{
	Player player;
	int current_angle = 0;
	...
	public HasteWaveEffect(Player _player, double _distance)
	{
		player = _player;
